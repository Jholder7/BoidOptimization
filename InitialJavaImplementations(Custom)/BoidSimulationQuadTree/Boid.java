...
// Original ideas explained by Daniel Shiffman (http://codingtra.in)
// Refactored and ported to work with my engine and java by Justin Holder
...
public class Boid extends Particle{
    ...
    public void onUpdate(float deltaTime){
        ...
        // Change instead of entire list we just pass local boids here simples enough right?
        // Instant start = Instant.now();
        BoundingRectangle Region = new BoundingRectangle(position.x, position.y, this.PERCEPTIONRADIUS, this.PERCEPTIONRADIUS);
        //System.out.println();
        flock(this.mainReference.boidQuadTree.query(Region, new LinkedList<Point>()));//boidQuadTree.getLocalBoids(this.position.x, this.position.y, this.PERCEPTIONRADIUS));
        // Instant end = Instant.now();
        // if (this.frameCount > 300){
        //     System.out.println("\r" + Duration.between(start, end));
        //     this.frameCount = 0;
        // } else {
        //     this.frameCount++;
        // }
    ...
    public void flock(LinkedList<Point> boids){
    ...
    private Vector2D separation(LinkedList<Point> boids){
        float perceptionRadius = this.PERCEPTIONRADIUS;
        int total = 0;

        Vector2D localAveragePosition = new Vector2D();
        for(Point other: boids){
            float distance = Vector2D.dist(this.position, other.boid.position);
            if (other.boid != this && distance < perceptionRadius){
    ...
    //TODO: Evaluate this function
    private Vector2D align(LinkedList<Point> boids){
        float perceptionRadius = this.PERCEPTIONRADIUS;
        int total = 0;
        ...
        for(Point other: boids){
            float distance = Vector2D.dist(this.position, other.boid.position);
            if (other.boid != this && distance < perceptionRadius){
                localAverageVelocity.add(other.boid.velocity);
                total++;
            }
        }
        if (total > 0) {
    ...
    //Move towards the average position of local flockmates
    //TODO: Evaluate this function
    private Vector2D cohesion(LinkedList<Point> boids){
        float perceptionRadius = this.PERCEPTIONRADIUS;
        int total = 0;
        ...
        for(Point other: boids){
            float distance = Vector2D.dist(this.position, other.boid.position);
            if (other.boid != this && distance < perceptionRadius){
                localAveragePosition.add(other.boid.position);
                total++;
            }
        }
        if (total > 0) {
